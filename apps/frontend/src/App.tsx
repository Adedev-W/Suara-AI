...
function App() {
  ...
  const cleanupAudio = () => { nodeRef.current?.disconnect(); sourceRef.current?.disconnect(); streamRef.current?.getTracks().forEach((track) => track.stop()); contextRef.current?.close(); nodeRef.current = null; sourceRef.current = null; streamRef.current = null; contextRef.current = null }
  const closeSocket = () => { socketRef.current?.close(); socketRef.current = null }
  const closeModal = () => { cleanupAudio(); closeSocket(); setIsModalOpen(false); setVoiceStage('idle') }
  ...
  const startAudio = async (socket: WebSocket) => {
    ...
    node.port.onmessage = (event: MessageEvent<Float32Array>) => { if (socket.readyState === WebSocket.OPEN) socket.send(encodePcm(event.data, context.sampleRate)) }
  ...
  const beginListening = () => {
    ...
    socket.onopen = () => { socket.send(JSON.stringify({ type: 'start' })); startAudio(socket).catch(() => { setError('Microphone access is required to search by voice.'); setVoiceStage('error') }) }
    socket.onmessage = (event) => { const message = JSON.parse(event.data) as { type: string; text?: string; final?: boolean; items?: SearchResult[]; message?: string }; if (message.type === 'listening') setVoiceStage('listening'); if (message.type === 'transcript' && message.text) { setTranscript(message.text); if (message.final) { cleanupAudio(); socket.send(JSON.stringify({ type: 'stop' })); setVoiceStage('transcript') } } if (message.type === 'results') { setResults(message.items ?? []); setVoiceStage('searching'); closeModal() } if (message.type === 'error') { setError(message.message ?? 'Voice search is temporarily unavailable.'); setVoiceStage('error') } }
